#!/bin/bash
# GuideLLM parallel benchmarking for vLLM Qwen3.6-27B (dense, hybrid DeltaNet+Attention)
...
#
# Defaults match the FP8 + MTP config (docker-compose.vllm-27b-fp8-rtx.yml)
# served on port 11436. Override via environment to test other variants:
#   PROCESSOR=mmangkad/Qwen3.6-27B-NVFP4 ./bench-guidellm-parallel.sh chat
#   PROCESSOR=Qwen/Qwen3.6-27B ./bench-guidellm-parallel.sh chat   # BF16
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/benchmarks/guidellm}"
...
main() {
  echo ""
  log "╔══════════════════════════════════════════════════════════════╗"
  log "║  GuideLLM Parallel Benchmark — vLLM Qwen3.6-27B (dense)    ║"
  log "║  GPU: RTX PRO 6000 Blackwell (96 GB)                       ║"
  log "╚══════════════════════════════════════════════════════════════╝"
  echo ""
