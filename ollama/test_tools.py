#!/usr/bin/env python3
"""
Tool-calling test for Ollama — verifies that the model correctly routes function calls.

Runs three scenarios:
  1. Single tool call  — get current weather
  2. Parallel tool calls — fetch weather for two cities simultaneously
  3. Chained tools     — tool result fed back; model produces a final answer

Usage:
    python test_tools.py
    python test_tools.py --base-url http://localhost:11434/v1 --scenario single
    python test_tools.py --model qwen3:30b --scenario chained
"""
...
def scenario_chained(client: OpenAI, model: str) -> bool:
...
def main():
    parser = argparse.ArgumentParser(description="Ollama tool-calling integration test")
